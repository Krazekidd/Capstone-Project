...
from schemas import (
    ...
    BookingResponse, MyBookingsResponse, CancelBookingResponse, MLScoreResponse, MLRecommendationsResponse
)
...
async def book_excursion(
    ...
    existing_booking = await db.execute(
        select(ExcursionBooking).where(
            and_(
                ExcursionBooking.user_id == user_id,
...
async def get_my_bookings(
    ...
    result = await db.execute(
        ...
        .where(ExcursionBooking.user_id == user_id)
...
async def cancel_booking(
    ...
    result = await db.execute(
        select(ExcursionBooking, Excursion)
        .join(Excursion, ExcursionBooking.excursion_id == Excursion.id)
        .where(ExcursionBooking.id == booking_id)
        .where(ExcursionBooking.user_id == user_id)
    )
    row = result.first()
    ...
    excursion_date = excursion.date.date()
