...
export default function Account() {
  ...
  const saveGoals = () => { setGoals({...goalInputs}); showToast("✓ Goals saved!"); };
  ...
  const askAI = async () => {
    ...
    } catch { setAiResponse("⚠️ Connection issue. Please try again."); }
    setAiLoading(false);
    setAiInput("");
  };
  ...
  const inp = (label, key, ph, step="1") => (
    <div className="field" key={key}>
      <label>{label}</label>
      <input type="number" placeholder={ph} step={step} value={m[key]} onChange={e => setM(p => ({...p, [key]: e.target.value}))} />
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="main">
        ...
        <div className="card" style={{ marginBottom:20 }}>
          ...
          <div className="form-row">
            <div className="field"><label>Gender</label><select value={gender} onChange={e => setGender(e.target.value)}><option value="male">Male</option><option value="female">Female</option></select></div>
            <div className="field"><label>Primary Goal</label><select><option>Build Muscle &amp; Size</option><option>Lose Fat &amp; Cut</option><option>Body Recomposition</option><option>Pure Strength</option><option>Endurance &amp; Cardio</option><option>Tone &amp; Define</option></select></div>
          </div>
          <div className="goal-body-select">
            {[["💪","Bulk Up"],["✂️","Cut Down"],["⚡","Athletic Build"],["🔥","Lean & Shredded"],["⚖️","Maintain"],["🌊","Tone & Define"]].map(([icon,name]) => (
              <div key={name} className={`goal-type-btn${goalType===name?" active":""}`} onClick={() => setGoalType(name)}><div className="g-icon">{icon}</div><div className="g-name">{name}</div></div>
            ))}
          </div>
          ...
          <div className="form-row-3">
            {[["Target Weight (kg)","weight"],["Target "+(gender==="female"?"Bust":"Chest")+" (cm)","chest"],["Target Waist (cm)","waist"]].map(([lbl,k]) => (
              <div className="field" key={k}><label>{lbl}</label><input type="number" value={goalInputs[k]} onChange={e => setGoalInputs(p => ({...p, [k]: +e.target.value}))} /></div>
            ))}
          </div>
          <div className="form-row-3">
        ...
        <div className="card" style={{ marginBottom:20 }}>
          ...
          <div className="tracker-layout">
            <div className="body-avatar-panel">
              ...
              <div className="avatar-svg-wrap">
                ...
                <div className="avatar-labels">
                  {[["Chest",av.chest],["Waist",av.waist],["Hips",av.hips],["Thigh",av.thigh],["Arm",av.arm]].map(([lbl,val]) => (
                    <div className="aml-row" key={lbl}><span className="aml-label">{lbl}</span><span className="aml-val">{m[lbl.toLowerCase()] ? val+" cm" : "—"}</span></div>
                  ))}
                </div>
                <div className="avatar-legend"><span><span className="legend-dot" style={{ background:"#ff6b1a" }} /> Updating live</span><span><span className="legend-dot" style={{ background:"rgba(240,238,234,0.35)" }} /> Goal</span></div>
              </div>
            </div>
            <div className="measure-section">
              ...
              <div className="measure-group"><div className="measure-group-title">📦 Body Basics</div><div className="measure-inputs">{inp("Weight (kg)","weight","84","0.1")}{inp("Height (cm)","height","178")}{inp("Body Fat %","bf","18","0.1")}</div></div>
              <div className="measure-group"><div className="measure-group-title">📐 Upper Body (cm)</div><div className="measure-inputs">{inp(gender==="female"?"Bust (cm)":"Chest (cm)","chest","96","0.5")}{inp("Waist (cm)","waist","84","0.5")}{inp("Shoulders (cm)","shoulders","118","0.5")}</div><div className="measure-inputs" style={{ marginTop:12 }}>{inp("Left Arm (cm)","armL","36","0.5")}{inp("Right Arm (cm)","armR","36","0.5")}{inp("Neck (cm)","neck","38","0.5")}</div></div>
              <div className="measure-group"><div className="measure-group-title">📐 Lower Body (cm)</div><div className="measure-inputs">{inp("Hips (cm)","hips","96","0.5")}{inp("Left Thigh (cm)","thighL","56","0.5")}{inp("Right Thigh (cm)","thighR","56","0.5")}</div><div className="measure-inputs" style={{ marginTop:12 }}>{inp("Left Calf (cm)","calfL","36","0.5")}{inp("Right Calf (cm)","calfR","36","0.5")}{inp("Glutes (cm)","glutes","100","0.5")}</div></div>
              <div style={{ display:"flex", gap:12 }}><button className="btn-primary" onClick={saveMeasurements}>💾 Save This Month</button><button className="btn-outline" onClick={clearM}>Clear</button></div>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"20px", flexWrap:"wrap" }}>
            ...
            <div>
              <div style={{ fontSize:24, fontWeight:"bold" }}>{waterLogs} / 8 cups</div>
              <button className="btn-primary" onClick={logWater} style={{ marginTop:12 }}>💧 Drink Now</button>
            </div>
          </div>
        </div>
        ...
        <div className="card" style={{ marginBottom:20 }}>
          ...
          <div style={{ display:chartTab==="compare"?"block":"none" }} className="compare-grid">
            {compareItems.map(item => {
              ...
              return (
                <div key={item.label}>
                  <div className="compare-row-top"><span className="c-label">{item.label}</span><div className="c-vals"><span className="c-current">Now: {item.current}{item.unit}</span><span className="c-goal">Goal: {item.goal}{item.unit}</span><span style={{ color:dc, fontWeight:700 }}>{+diff>0?"+":""}{diff}</span></div></div>
                  <div className="compare-bars"><div className="bar-goal" style={{ width:`${gp}%` }} /><div className="bar-current" style={{ width:`${cp}%` }} /></div>
                </div>
              );
            })}
          </div>
          <div style={{ display:chartTab==="weight"?"block":"none" }}><div className="chart-canvas-wrap"><WeightChart history={history} goalWeight={goals.weight} /></div></div>
          <div style={{ display:chartTab==="body"?"block":"none" }}><div className="chart-canvas-wrap"><BodyChart history={history} /></div></div>
          <div style={{ display:chartTab==="strength"?"block":"none" }}><div className="chart-canvas-wrap"><StrengthChart /></div></div>
        </div>
        ...
        <div className="card">
          ...
          <p style={{ fontSize:13, color:"#6b6b7a", marginBottom:16 }}>
            Rate each trainer you've worked with. Your feedback helps us improve.
          </p>
          ...
          <div className="avg-rating" style={{ marginTop: 20, justifyContent: 'space-between' }}>
          ...
          <div className="card grid-full">
            ...
            <div className="target-grid">
              {[["💪","Chest & Arms","chest","Push power & definition"],["🦴","Back & Lats","back","Width & thickness"],["🦵","Legs & Glutes","legs","Strength & size"],["🎯","Core & Abs","core","Stability & tone"],["⚡","Full Body","fullbody","Total conditioning"],["🏃","Cardio & HIIT","cardio","Burn & endurance"]].map(([icon,name,key,desc]) => (
                <button key={key} className={`target-btn${activeTarget===key?" active":""}`} onClick={() => setActiveTarget(key)}><div className="icon">{icon}</div><div className="name">{name}</div><div className="desc">{desc}</div></button>
              ))}
            </div>
            {activeTarget && (
              <div className="workout-recs show"><div style={{ margin:"20px 0 14px", fontFamily:"'Bebas Neue',sans-serif", fontSize:18, letterSpacing:2, color:"#ff6b1a" }}>RECOMMENDED WORKOUTS</div><div className="workout-grid">{WORKOUTS[activeTarget].map(w => (
                <div className="workout-card" key={w.name}><div className="w-icon">{w.icon}</div><div className="w-name">{w.name}</div><div className="w-detail">{w.detail}</div><div className="w-sets">{w.sets.map(s => <span className="set-tag" key={s}>{s}</span>)}</div></div>
              ))}</div></div>
            )}
          </div>
          <div className="card">
            ...
            <div style={{ marginTop:18 }}>{[["Bench Press","100/120kg",83],["Squat","130/150kg",87],["Deadlift","160/180kg",89],["Overhead Press","65/80kg",81],["Pull-ups","18/25 reps",72]].map(([name,val,pct]) => (
              <div className="progress-item" key={name}><div className="progress-top"><span>{name}</span><span>{val}</span></div><div className="progress-bar"><div className="progress-fill" style={{ width:`${pct}%` }} /></div></div>
            ))}</div>
          </div>
          <div className="card">
            ...
            <div style={{ marginTop:18 }}>
              <div className="schedule-row">{[["Mon",25,true,false],["Tue",26,false,false],["Wed",27,true,false],["Thu",28,true,false],["Fri",1,false,false],["Sat",2,true,true],["Sun",3,false,false],["Mon",4,true,false]].map(([day,num,dot,today]) => (
                <div key={`${day}${num}`} className={`day-card${today?" today":""}`}><div className="day-name">{day}</div><div className="day-num">{num}</div>{dot?<div className="day-dot" />:<div className="day-empty" />}</div>
              ))}</div>
      ...
      {showSettings && createPortal(
        <div className="modal-overlay" onClick={() => setShowSettings(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <input type="color" value={tempColor} onChange={e => setTempColor(e.target.value)} style={{ width:"100%", margin:"15px 0", padding:"8px" }} />
      ...
      {/* Chat FAB */}
      <div className="chat-fab"><span className="chat-label">Live Support</span><button className="chat-btn" onClick={() => setChatOpen(o => !o)}>💬<div className="chat-ping" /></button></div>
      {chatOpen && createPortal(
        <div className="chat-popup" style={{ display:"flex" }}>
          <div className="chat-header"><div className="chat-header-info"><div className="chat-avatar">🏋️</div><div><div className="name">GymPro Support</div><div className="sub">Online now</div></div></div><button className="close-btn" onClick={() => setChatOpen(false)}>✕</button></div>
          <div className="chat-messages" ref={chatMsgsRef}>{chatMessages.map((msg,i) => <div key={i} className={`msg ${msg.role}`}>{msg.text}<div className="msg-time">{msg.time}</div></div>)}</div>
          <div className="chat-input-area"><input className="chat-input" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key==="Enter" && sendChat()} placeholder="Type a message..." /><button className="send-btn" onClick={sendChat}>➤</button></div>
        </div>, document.body)}
    </>
  );
}