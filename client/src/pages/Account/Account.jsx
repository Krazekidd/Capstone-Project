...
const chartOptions = {
  ...
  useEffect(() => {
    ...
    if (!history || history.length === 0) {
      ...
      ctx.fillText("No weight data available. Start tracking your progress!", canvas.width / 2, canvas.height / 2);
      return;
    }
    
    // Filter out entries with missing weight data
    const validHistory = history.filter(h => h.weight && h.weight > 0);
    ...
    const labels = validHistory.map(h => `${h.month} ${h.year || ''}`);
    const weightData = validHistory.map(h => h.weight);
      ...
      chart.current = new Chart(ref.current, {
        ...
        data: {
          ...
          datasets: [
            ...
            {
              label: "Goal",
              data: validHistory.map(() => goalWeight),
  ...
  return <canvas ref={ref} style={{ width: '100%', height: '100%', minHeight: '260px' }} />;
}
...
function BodyChart({ history }) {
  ...
  useEffect(() => {
    ...
    if (!history || history.length === 0) {
      ...
      ctx.font = "14px 'DM Sans', sans-serif";
      ctx.textAlign = "center";
      ctx.fillText("No body measurement data available. Start tracking your progress!", canvas.width / 2, canvas.height / 2);
      return;
    }
    ...
    setTimeout(() => {
      ...
      chart.current = new Chart(ref.current, {
        ...
        data: {
          labels: labels,
          datasets: [
            { label: "Chest/Bust", data: history.map(h => h.chest), borderColor: "#ff6b1a", tension: 0.4, borderWidth: 2, fill: false, pointRadius: 4, pointHoverRadius: 6 },
            { label: "Waist", data: history.map(h => h.waist), borderColor: "#ffa040", tension: 0.4, borderWidth: 2, fill: false, pointRadius: 4, pointHoverRadius: 6 },
            { label: "Hips", data: history.map(h => h.hips), borderColor: "#f0eeea", tension: 0.4, borderWidth: 2, fill: false, pointRadius: 4, pointHoverRadius: 6 },
            { label: "Thigh", data: history.map(h => h.thigh), borderColor: "#6b6b7a", tension: 0.4, borderWidth: 2, fill: false, pointRadius: 4, pointHoverRadius: 6 },
            { label: "Arm", data: history.map(h => h.arm), borderColor: "#d4a050", tension: 0.4, borderWidth: 2, fill: false, pointRadius: 4, pointHoverRadius: 6 },
          ],
        },
        options: {
          ...chartOptions,
...
function StrengthChart({ strengthRecords }) {
  const ref = useRef(null);
  const chart = useRef(null);
  ...
  useEffect(() => {
    ...
    exercises.forEach(exercise => {
      const record = strengthRecords?.find(r => r.exercise_name === exercise);
      currentData.push(record?.current_weight_kg || 0);
      goalData.push(record?.goal_weight_kg || 0);
    });
    ...
    chart.current = new Chart(ref.current, {
      ...
      data: {
        labels: exercises,
        datasets: [
          { label: "Current (kg)", data: currentData, backgroundColor: "rgba(255,107,26,0.75)", borderRadius: 6 },
          { label: "Goal (kg)", data: goalData, backgroundColor: "rgba(240,238,234,0.14)", borderRadius: 6, borderWidth: 1, borderColor: "rgba(240,238,234,0.38)" },
        ],
      },
      options: { ...chartOptions, barPercentage: 0.6, scales: { ...chartOptions.scales, y: { beginAtZero: true } } },
    });
...
export default function Account() {
  ...
  const [healthOptions] = useState(["Diabetes", "Hypertension", "Asthma", "Knee Injury", "Back Pain"]);
  ...
  const [trainersList] = useState(["Sherice Myers", "Ashton Hall"]);
  ...
  const loadAllUserData = async () => {
    try {
      ...
      // Load goals from client_goals table
      await loadGoals();
      
      // Load progress history from body_measurements and progress_tracking
      await loadProgressHistory();
      
      // Load health conditions
      await loadHealthConditions();
      
      // Load water intake from client_water_intake
  ...
  try {
    ...
    if (historyData && historyData.length > 0) {
      const formattedHistory = historyData.map(entry => {
        ...
        const recordDate = entry.recorded_at ? new Date(entry.recorded_at) : new Date();
        
        return {
          month: MONTHS[recordDate.getMonth()],
      ...
      // Set current measurements from latest entry
...
const loadGoals = async () => {
  try {
    ...
    if (goalsData && goalsData.target_weight_kg) {
      const newGoals = {
      ...
      if (goalsData.goal_type) setGoalType(goalsData.goal_type);
    } else {
      // Try to get from client_goals table directly
      const fallbackGoals = {
        weight: 80,
        chest: 100,
        waist: 80,
    ...
    setGoalInputs(defaultGoals);
  }
};
  const loadHealthConditions = async () => {
    try {
      const conditions = await progressAPI.getHealthConditions();
      setHealthConditions(conditions.map(c => c.condition_name));
    } catch (err) {
  ...
  const loadWaterIntake = async () => {
    try {
      const intake = await progressAPI.getWaterIntake();
      setWaterLogs(intake.cups_consumed || 0);
    } catch (err) {
      console.log("Water intake not loaded");
    }
  };
  
  const loadStrengthRecords = async () => {
    try {
      const records = await progressAPI.getStrengthRecords();
      setStrengthRecords(records || []);
    } catch (err) {
      console.log("Strength records not loaded");
    }
  };
  
  const loadTrainerRatings = async () => {
    try {
      const ratings = await progressAPI.getTrainerRatings();
      const ratingsMap = {};
      ratings.ratings?.forEach(r => {
        ratingsMap[r.trainer_name] = r.rating;
      });
      setTrainerRatings(ratingsMap);
    } catch (err) {
      console.log("Trainer ratings not loaded");
    }
  };
  ...
  const loadTrainingSchedule = async () => {
    try {
      const schedule = await progressAPI.getTrainingSchedule();
      setTrainingSchedule(schedule || []);
    } catch (err) {
      console.log("Training schedule not loaded");
    }
  };
  ...
  const logWater = async () => {
    if (waterLogs < 8) {
      const newCount = waterLogs + 1;
      setWaterLogs(newCount);
      try {
        await progressAPI.logWaterIntake(newCount);
        showToast(`💧 Water logged! ${newCount}/8 cups today.`);
      } catch (err) {
        console.error("Failed to log water:", err);
        showToast("Failed to log water intake");
      }
    } else {
  ...
  const handleRateTrainer = async (trainer, rating) => {
    try {
      await progressAPI.rateTrainer(trainer, rating);
  ...
  const saveHealthConditions = async () => {
    try {
      await progressAPI.updateHealthConditions(healthConditions);
      showToast("✓ Health conditions saved!");
    } catch (err) {
      console.error("Error saving health conditions:", err);
      showToast("Failed to save health conditions");
    }
  };
  ...
  const saveMeasurements = async () => {
    try {
      ...
      // Prepare complete measurements object for body_measurements table
      ...
      Object.keys(measurementsData).forEach(key => {
        if (measurementsData[key] === null) delete measurementsData[key];
      });
      
      // Save to backend (body_measurements table)
      await progressAPI.saveProgress(measurementsData);
      
      // Refresh history to get updated data
      await refreshData();
      
      // Create new entry for local history
      const newEntry = {
        month: currentMonth,
        year: currentYear,
  ...
  const todaySchedule = trainingSchedule.find(s => s.is_today);
    ...
    <>
      ...
      <div className="main">
        ...
        <div className="card">
          ...
          <div className="health-checkbox-group">
            {healthOptions.map(cond => (
              <label key={cond} className="health-checkbox">
                <input type="checkbox" checked={healthConditions.includes(cond)} onChange={e => {
          ...
          <button className="btn-primary" style={{ marginTop: 16 }} onClick={saveHealthConditions}>Save Health Info</button>
        </div>
        
        {/* Track Your Progress Card - Saves to body_measurements and progress_tracking */}
        <div className="card" style={{ marginBottom: 20 }}>
          ...
          <div className="tracker-layout">
            ...
            <div className="measure-section">
              <div className="measure-header">
                <span className="measure-title">{MONTHS[new Date().getMonth()]} {new Date().getFullYear()} Entry</span>
              ...
              <div className="measure-actions">
                ...
                <button className="btn-outline" onClick={clearMeasurements}>Clear</button>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Progress Dashboard - Uses goals and latest measurements */}
        <div className="card" style={{ marginBottom: 20 }}>
  ...
  <div style={{ position: 'relative', height: '300px', width: '100%' }}>
    <BodyChart history={history} />
          ...
          <div style={{ display: chartTab === "strength" ? "block" : "none" }} className="chart-canvas-wrap">
            <StrengthChart strengthRecords={strengthRecords} />
          </div>
        </div>
          ...
          <div className="card">
            ...
            <div className="strength-stats">
              {strengthRecords.map(record => {
                const progress = record.percentage_progress || Math.round((record.current_weight_kg / record.goal_weight_kg) * 100);
                return (
                  <div className="progress-item" key={record.exercise_name}>
                    <div className="progress-top">
                      <span>{record.exercise_name}</span>
                      <span>{record.current_weight_kg}/{record.goal_weight_kg}kg</span>
                    </div>
              ...
              {strengthRecords.length === 0 && (
                <p style={{ textAlign: "center", color: "#6b6b7a", padding: "20px" }}>No strength records yet. Start tracking your lifts!</p>
              )}
            </div>
          </div>
          <div className="card">
            ...
            <div className="schedule-container">
              <div className="schedule-row">
                {trainingSchedule.slice(0, 8).map(day => (
                  <div key={day.id} className={`day-card${day.is_today ? " today" : ""}`}>
                    <div className="day-name">{day.day_of_week?.substring(0, 3)}</div>
                    <div className="day-num">{day.day_number}</div>
                    {day.has_session ? <div className="day-dot" /> : <div className="day-empty" />}
                  </div>
                ))}
              </div>
              {todaySchedule && (
                <div className="today-session">
                  ...
                  <div className="today-name">{todaySchedule.session_name || "Rest Day"}</div>
                  {todaySchedule.session_time && (
                    <div className="today-detail">{todaySchedule.session_time} · {todaySchedule.session_name}</div>
                  )}
                </div>
              )}
            </div>
        ...
        <div className="card">
          ...
          <div className="badges-grid">
            {badges.map(badge => (
              <div className="badge-item" key={badge.id}>{badge.badge_name}</div>
            ))}
            {badges.length === 0 && (
              <p style={{ textAlign: "center", color: "#6b6b7a", gridColumn: "1/-1" }}>Complete workouts to earn badges!</p>
            )}
          </div>
        </div>
      </div>
