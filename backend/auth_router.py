...
async def register_client(
    ...
    new_user = User(
        email=request.email, password_hash=str(hashed, "utf-8"), role="client"
    )
    db.add(new_user)
    await db.flush()
    ...
    return TokenResponse(
        access_token=token, token_type="bearer", role="client", user_id=user_uuid
    )
...
async def login(request: LoginRequest, db: AsyncSession = Depends(get_user_db)):
    ...
    return TokenResponse(
        access_token=token, token_type="bearer", role=user.role, user_id=user_uuid
    )
...
async def forgot_password(
    ...
    client_result = await db.execute(select(Client.name).where(Client.id == user.id))
...
async def google_login(
    ...
    try:
        ...
        if not user:
            ...
            # Create client profile
            new_client = Client(id=new_user.id, name=name)
            db.add(new_client)
            await db.commit()
            await db.refresh(new_user)
        ...
        return TokenResponse(
            access_token=token, token_type="bearer", role=role, user_id=user_id
        )
...
async def get_user_info(
    ...
    user_info = {"id": str(user_id), "email": user.email, "role": role}
    ...
    if role == "client":
        ...
        admin_result = await db.execute(select(Admin).where(Admin.id == user_id_bytes))
