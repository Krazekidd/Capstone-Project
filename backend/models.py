...
class SavedConversation(Base):
    ...
    title: Mapped[str] = mapped_column(
        String(255), nullable=False, default="Untitled Chat"
    )
...
class User(Base):
    ...
    id = Column(LargeBinary(16), primary_key=True, default=generate_uuid)
    ...
    progress_entries = relationship(
        "ProgressTracking", back_populates="user", cascade="all, delete-orphan"
    )
...
class Client(Base):
    ...
    id = Column(LargeBinary(16), ForeignKey("Accounts.id"), primary_key=True)
...
class ProgressTracking(Base):
    ...
    id = Column(LargeBinary(16), primary_key=True, default=generate_uuid)
    user_id = Column(LargeBinary(16), ForeignKey("Accounts.id"), nullable=False)
...
class ClientGoal(Base):
    ...
    client_id = Column(
        LargeBinary(16), ForeignKey("clients.id", ondelete="CASCADE"), nullable=False
    )
...
class PasswordResetToken(Base):
    ...
    user_id = Column(
        LargeBinary(16), ForeignKey("Accounts.id", ondelete="CASCADE"), nullable=False
    )
...
class Excursion(Base):
    ...
    # Relationships
    tags = relationship(
        "ExcursionTag", back_populates="excursion", cascade="all, delete-orphan"
    )
    bring_items = relationship(
        "ExcursionBringItem", back_populates="excursion", cascade="all, delete-orphan"
    )
...
class ExcursionTag(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    excursion_id = Column(
        String(50), ForeignKey("excursions.id", ondelete="CASCADE"), nullable=False
    )
...
class ExcursionBooking(Base):
    ...
    excursion_id = Column(
        String(50), ForeignKey("excursions.id", ondelete="RESTRICT"), nullable=False
    )
...
class ConsultationBooking(Base):
    ...
    consultation_type_id = Column(
        String(50), ForeignKey("consultation_types.id"), nullable=False
    )
    booking_reference = Column(String(50), unique=True, nullable=False)
...
class ConsultationAvailability(Base):
    ...
    booked_by = Column(
        LargeBinary(16), ForeignKey("clients.id", ondelete="SET NULL"), nullable=True
    )
    booking_id = Column(
        LargeBinary(16),
...
class ShopCartItem(Base):
    ...
    product_id = Column(
        String(50), ForeignKey("shop_products.id", ondelete="CASCADE"), nullable=False
    )
...
class ShopOrderItem(Base):
    ...
    product_id = Column(
        String(50), ForeignKey("shop_products.id", ondelete="RESTRICT"), nullable=False
    )
...
class TrainerAssessment(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    trainer_id = Column(
        LargeBinary(16), ForeignKey("trainers.id", ondelete="CASCADE"), nullable=False
    )
    trainer_name = Column(String(100), nullable=False)
...
class ClientStatus(Base):
    ...
    assigned_trainer_id = Column(
        LargeBinary(16), ForeignKey("trainers.id", ondelete="SET NULL"), nullable=True
    )
